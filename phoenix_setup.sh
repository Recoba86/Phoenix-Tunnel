...
EOL
    ...
    SERVER_PUB_KEY=$(sshpass -p "$FOREIGN_PASS" ssh -n -o StrictHostKeyChecking=no -p $FOREIGN_PORT root@$FOREIGN_IP "cat /opt/phoenix/server_keys_${CONN_NAME}.txt" | grep 'Public Key:' | awk '{print $3}')
